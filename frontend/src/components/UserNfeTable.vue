      ...
      <DefaultTableRow :value="value" :key-index="keyIndex">
        <template #cell="{ value, keyIndex }">
          <div v-if="isButtonCell(keyIndex)" class="flex w-full pe-1">
            <button
              @click="value"
              class="rounded-3xl border-solid border hover:bg-p-blue border-p-blue px-7 py-2 text-n-dark-400 ms-auto font-bold text-sm leading-4"
            >
              Dados do cedente
            </button>
          </div>
          <span v-else-if="isValueCell(keyIndex)" class="text-cash-green font-medium">{{
            value
          }}</span>
          <span
