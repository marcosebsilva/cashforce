...
<template>
  <table class="table-auto w-full border-separate border-spacing-y-3">
    <thead class="ml-2">
      <tr>
        <th class="text-start text-n-mid-1 pl-8 text-xs text-n-mid-200" v-for="header in props.headers">{{ header.toUpperCase() }}</th>
      </tr>
    </thead>
    <tbody>
      <slot name="row" v-for="(item, keyIndex) in Object.values(props.content)" :value=item.value :key-index=keyIndex />
    </tbody>
  </table>
</template>